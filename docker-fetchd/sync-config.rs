...
const BASE_URL: &str =
    "https://raw.githubusercontent.com/fetchai/genesis-fetchhub/fetchhub-4/fetchhub-4/data";

#[derive(Parser, Debug)]
#[command(name = "sync-config")]
#[command(about = "Sync config files for fetchd", long_about = None)]
...
fn main() -> Result<()> {
    ...
    download_file(
        &format!("{BASE_URL}/genesis_migrated_5300200.json"),
        &base_dir.join("config/genesis.json"),
        args.dry_run,
    )?;

    // Compress genesis.json with xz
    let genesis_path = base_dir.join("config/genesis.json");
    println!();
    println!(
        "{} {} {}",
        "→".bold().cyan(),
        "Compressing".bold(),
        "config/genesis.json".yellow()
    );
    ...
    if args.dry_run {
        println!(
            "  {} {}",
            "[DRY RUN]".bold().yellow(),
            format!("xz -9 {}", genesis_path.display()).dimmed()
        );
    } else {
        ...
        if !status.success() {
            eprintln!("  {} Failed to compress genesis.json", "✗".bold().red());
            anyhow::bail!("xz compression failed");
        }
        println!("  {} Compressed successfully", "✓".bold().green());
    }
