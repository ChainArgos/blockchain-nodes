...
const BASE_URL: &str = "https://raw.githubusercontent.com/tronprotocol/java-tron/refs/heads/master/framework/src/main/resources";

#[derive(Parser, Debug)]
#[command(name = "sync-config")]
#[command(about = "Sync config files for tron-java", long_about = None)]
...
fn main() -> Result<()> {
    ...
    println!(
        ...
        "tron-java".bold().green(),
    );
    println!("{}", "━".repeat(60).bright_black());

    download_file(
        &format!("{BASE_URL}/config.conf"),
        &base_dir.join("config/main_net_config.conf"),
        args.dry_run,
    )?;
