...
const BASE_URL: &str = "https://docs.linea.build/files/geth/mainnet";

#[derive(Parser, Debug)]
#[command(name = "sync-config")]
#[command(about = "Sync config files for geth (linea)", long_about = None)]
...
fn main() -> Result<()> {
    ...
    println!(
        ...
        "geth (linea)".bold().green(),
    );
    println!("{}", "━".repeat(60).bright_black());

    // Create config/linea/ directory
    let linea_dir = base_dir.join("config/linea");
    if !args.dry_run {
        std::fs::create_dir_all(&linea_dir)
            .with_context(|| format!("Failed to create directory: {}", linea_dir.display()))?;
    }

    download_file(
        &format!("{BASE_URL}/genesis.json"),
        &base_dir.join("config/linea/genesis.json"),
        args.dry_run,
    )?;
